...
// It is included for comparisons.  
...
void cut3c(int N, int M, int depthLimit) {
  ...
  */
  ...
  void cut3c();
  if ( 0 <= dupx ) {
    cut3duplicates(N, M, dupx, cut3c, depthLimit);
    return;
  }
...
Start:
 ...
 Right:
	...
	A[i] = x;
	if ( i - N < M - j) {
	  cut3c(N, i-1, depthLimit);
	  N = i;
	  goto Begin;
	}
	cut3c(i, M, depthLimit);
	M = i - 1;
	goto Begin;
